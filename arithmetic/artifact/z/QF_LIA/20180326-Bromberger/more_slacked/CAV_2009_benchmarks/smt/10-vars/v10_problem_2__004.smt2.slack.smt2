...
(assert (>= x0_plus 0))
(assert (>= x0_minus 0))
(assert (<= (+ (* 2 x8_plus) (* (- 2) x8_minus) (* 2 x5_plus) (* (- 2) x5_minus) (* (- 1) x3_plus) x3_minus ) 1 ) )
(assert (<= (+ x9_plus (* (- 1) x9_minus) (* (- 2) x3_plus) (* 2 x3_minus) ) 0 ) )
(assert (>= (+ x9_plus (* (- 1) x9_minus) (* (- 1) x7_plus) x7_minus (* (- 2) x6_plus) (* 2 x6_minus) x5_plus (* (- 1) x5_minus) x3_plus (* (- 1) x3_minus) (* (- 1) x1_plus) x1_minus ) 0 ) )
(assert (>= (+ x3_plus (* (- 1) x3_minus) (* (- 1) x2_plus) x2_minus (* (- 1) x0_plus) x0_minus ) (- 1) ) )
