...
(assert (<= (+ x7_plus (* (- 1) x7_minus) (* (- 1) x5_plus) x5_minus x2_plus (* (- 1) x2_minus) (* (- 1) x1_plus) x1_minus ) 1 ) )
(assert (<= (+ x9_plus (* (- 1) x9_minus) (* (- 1) x6_plus) x6_minus (* (- 1) x4_plus) x4_minus (* (- 1) x0_plus) x0_minus ) 1 ) )
(assert (<= (+ x6_plus (* (- 1) x6_minus) x4_plus (* (- 1) x4_minus) x3_plus (* (- 1) x3_minus) x1_plus (* (- 1) x1_minus) ) (- 1) ) )
(assert (<= (+ x8_plus (* (- 1) x8_minus) (* (- 1) x7_plus) x7_minus (* (- 1) x2_plus) x2_minus ) 0 ) )
(check-sat)
(exit)
