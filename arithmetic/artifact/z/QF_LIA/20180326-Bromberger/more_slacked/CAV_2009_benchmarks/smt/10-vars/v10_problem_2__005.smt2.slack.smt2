...
(assert (>= x3_plus 0))
(assert (>= x3_minus 0))
(assert (<= (+ x9_plus (* (- 1) x9_minus) x8_plus (* (- 1) x8_minus) (* (- 1) x7_plus) x7_minus x6_plus (* (- 1) x6_minus) (* 2 x4_plus) (* (- 2) x4_minus) ) (- 1) ) )
(assert (<= (+ x6_plus (* (- 1) x6_minus) x5_plus (* (- 1) x5_minus) x2_plus (* (- 1) x2_minus) (* (- 2) x1_plus) (* 2 x1_minus) ) 0 ) )
(assert (<= (+ x9_plus (* (- 1) x9_minus) (* (- 1) x8_plus) x8_minus x2_plus (* (- 1) x2_minus) ) 0 ) )
(assert (>= (+ x9_plus (* (- 1) x9_minus) (* (- 2) x8_plus) (* 2 x8_minus) x7_plus (* (- 1) x7_minus) (* (- 1) x6_plus) x6_minus x4_plus (* (- 1) x4_minus) x1_plus (* (- 1) x1_minus) ) 1 ) )
(assert (>= (+ (* 2 x8_plus) (* (- 2) x8_minus) (* (- 1) x4_plus) x4_minus (* (- 2) x3_plus) (* 2 x3_minus) x2_plus (* (- 1) x2_minus) ) 0 ) )
(check-sat)
(exit)
