...
(assert (>= x3_plus 0))
(assert (>= x3_minus 0))
(assert (<= (+ x8_plus (* (- 1) x8_minus) x7_plus (* (- 1) x7_minus) x1_plus (* (- 1) x1_minus) x0_plus (* (- 1) x0_minus) ) 0 ) )
(assert (<= (+ x4_plus (* (- 1) x4_minus) x2_plus (* (- 1) x2_minus) x0_plus (* (- 1) x0_minus) ) 0 ) )
(assert (<= (+ x7_plus (* (- 1) x7_minus) (* (- 1) x4_plus) x4_minus (* (- 1) x3_plus) x3_minus ) (- 1) ) )
(assert (<= (+ (* 2 x7_plus) (* (- 2) x7_minus) x3_plus (* (- 1) x3_minus) ) 0 ) )
