...
(assert (<= (+ x4_plus (* (- 1) x4_minus) x3_plus (* (- 1) x3_minus) x2_plus (* (- 1) x2_minus) (* (- 1) x1_plus) x1_minus ) 0 ) )
(assert (<= (+ x8_plus (* (- 1) x8_minus) (* (- 1) x2_plus) x2_minus x1_plus (* (- 1) x1_minus) ) 1 ) )
(assert (<= (+ (* 2 x2_plus) (* (- 2) x2_minus) x1_plus (* (- 1) x1_minus) ) 1 ) )
(assert (>= (+ x9_plus (* (- 1) x9_minus) x7_plus (* (- 1) x7_minus) x6_plus (* (- 1) x6_minus) (* (- 1) x2_plus) x2_minus (* (- 1) x1_plus) x1_minus ) 1 ) )
(check-sat)
(exit)
