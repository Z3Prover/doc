...
(assert (>= x9_minus 0))
(assert (>= x9_plus 0))
(assert (>= (+ x2_plus x8_plus (- x2_minus) (- x8_minus)) 0))
(assert (>= (+ (- x3_plus) (* (- 2) x7_plus) x3_minus (* 2 x7_minus)) 0))
(assert (>= (+ x3_plus x4_plus (- x7_plus) (- x3_minus) (- x4_minus) x7_minus) 1))
(assert (>= (+ (- x0_plus) (- x2_plus) (- x4_plus) x0_minus x2_minus x4_minus) 0))
(assert (>= (+ (- x0_plus) (- x1_plus) (- x7_plus) (- x8_plus) x0_minus x1_minus x7_minus x8_minus) 0))
(check-sat)
(exit)
